  ...
  <div class="order-item-group">
      ...
      <div class="goods-group">
        <ul>
          <li v-for="order_item in order.order_items" :key="order_item.id">
            <span class="goods-name">{{order_item.goods.name}}</span>
            <span class="goods-amount">×{{order_item.count}}</span>
          </li>
        </ul>
      </div>
      <div class="order-info">
        <span class="order-create-time">{{order.create_time}}</span>
    ...
    <div class="order-footer">
      ...
      <div class="button-group" v-else-if="status == 2">
        <van-button type="warning" class="repeat-order">催单</van-button>
        <van-button type="default" class="comment">取消订单</van-button>
      </div>
